...
--Special queries based upon cte
--1--Problem Statement :- Write a SQL query to find all levels of Employee Manager hierarchy based upon this table
...
with cte_a as(
select employeeid,employeename,managerid,0 as EmployeeLevel from Employee_Table
where ManagerID is null 
union all
select emp.employeeid,emp.employeename,emp.managerid,EmployeeLevel+1  from Employee_Table as emp
inner join cte_a as mgr on  emp.ManagerID=mgr.EmployeeID
)
select * from cte_a
order by EmployeeLevel

--2----Input :- SampleTable has three columns namely X, Y and Z
